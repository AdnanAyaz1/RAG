...
Feature Name:
Tenant Dashboard & Document Management
...
Backend pushes processing job to Redis/BullMQ queue ->
Frontend shows processing progress with status badge ->
Backend RAG microservice processes document (parse -> chunk -> embed -> Pinecone index) ->
...
RAG microservice embeds query, searches Pinecone (tenant namespace), calls OpenAI LLM ->
Answer with citations returned to frontend ->
...
| Page | Purpose |
|------|---------|
| /login | Email/password authentication form with validation and error messages |
| /signup | Tenant registration with tenant name, admin email, password, display name |
| /dashboard | Tenant overview: storage usage, recent activity feed, quick access to docs and search |
| /documents | Document list with status badges, upload button, search/filter, pagination |
| /documents/:id | Document detail with status, processing progress, error messages |
| /search | Natural language query input, RAG answer display with source citations |
| /conversations | List of past search sessions with date, query preview, and response preview |
| /conversations/:id | Full conversation view with all questions and answers |
| /team | Tenant member management with role assignment |
| /invitations | Pending invitations and invite history |
| /settings | Tenant configuration (chunk size, embedding model preferences) |
| /api-keys | API key management (create, revoke, display key once) |
| /profile | User profile update and password change |
| /invite/accept/:token | Invitation acceptance page with role selection |
...
| ProtectedRoute | Middleware that checks JWT and redirects to login if missing or expired |
| AuthForm | Login form with email/password fields, validation, error display, remember-me toggle |
| SignupForm | Tenant registration form with tenant name, admin details, password strength indicator |
...
| SearchInput | Query input field with auto-focus, command-line style UX, enter-to-submit |
| AnswerCard | Displays RAG answer with source citations as clickable document links |
| SourceLink | Shows document name and chunk preview, links to document detail page |
| Toast | Animated notification (success/error/info/warning) with auto-dismiss and manual close |
| ConfirmDialog | Modal dialog for destructive actions (delete document, remove user, revoke API key) |
| LoadingSpinner | Full-page overlay spinner during page loads, inline spinner for table/skeleton loading |
| EmptyState | Illustrated placeholder for empty lists with action button (e.g., "No documents yet — upload one") |
| ErrorBoundary | React error boundary that catches component errors and displays a branded fallback UI |
| Pagination | Page controls with prev/next and page number buttons, items-per-page selector |
| SidebarNav | Role-aware sidebar menu that shows/hides navigation items based on user role |

---

# 5. API Integration

| Endpoint | Purpose | Method | React Query Hook |
|----------|---------|--------|-----------------|
| POST /auth/signup | Create tenant and admin account | POST | useSignupMutation |
| POST /auth/login | Authenticate and receive JWT tokens | POST | useLoginMutation |
| POST /auth/refresh | Refresh expired access token | POST | useRefreshMutation |
| POST /auth/logout | Invalidate refresh token | POST | useLogoutMutation |
| GET /tenants/me | Get current tenant profile | GET | useTenantQuery |
| PATCH /tenants/me | Update tenant settings | PATCH | useUpdateTenantMutation |
| GET /tenants/me/members | List tenant members | GET | useMembersQuery |
| POST /tenants/me/invitations | Invite user to tenant | POST | useInviteMutation |
| POST /documents/upload | Upload a new document file | POST (multipart) | useUploadDocumentMutation |
| GET /documents | List tenant documents (paginated) | GET | useDocumentsQuery |
| GET /documents/:id | Get document details | GET | useDocumentQuery |
| DELETE /documents/:id | Delete/archived a document | DELETE | useDeleteDocumentMutation |
| POST /documents/:id/reprocess | Trigger document reprocessing | POST | useReprocessMutation |
| POST /search | Submit query to RAG pipeline | POST | useSearchMutation |
| GET /search/history | List past queries for tenant | GET | useSearchHistoryQuery |
| GET /conversations | List conversation sessions | GET | useConversationsQuery |
| POST /conversations | Create new conversation session | POST | useCreateConversationMutation |
| GET /api-keys | List API keys for tenant | GET | useApiKeysQuery |
| POST /api-keys | Create new API key | POST | useCreateApiKeyMutation |
| DELETE /api-keys/:id | Revoke an API key | DELETE | useRevokeApiKeyMutation |
| GET /users/me | Get current user profile | GET | useCurrentUserQuery |
| PATCH /users/me | Update user profile | PATCH | useUpdateUserMutation |
...
- Modal visibility state (delete confirmation, invite dialog, API key reveal)
...
**Why:** Zustand provides lightweight global state for auth and tenant context that persists across page navigations and survives React re-renders. React Query manages all server state with automatic caching, background refetching, stale-while-revalidate, optimistic updates for mutations, and request deduplication. This separation keeps global state minimal (only auth/tenant context) while server state is fully synchronized with the backend with minimal boilerplate.
...
- Tenant settings update (chunk size, embedding model)
...
- File type validated on picker (accept PDF, DOCX, TXX)
...
- All form errors displayed inline next to the relevant field, not as toast
...
- 403 Forbidden shows permission-denied toast with role explanation
- 422 Unprocessable Entity displays field-level validation messages from backend Pydantic schema
- 429 Too Many Requests shows "Please wait a moment before trying again" message with countdown
- 500 Internal Server Error shows generic error with option to retry and report
- Network errors (offline, timeout) detected with offline-aware messaging suggesting retry
- Error boundary catches unexpected rendering errors and shows a "Something went wrong" screen with a reload button
...
### Tablet (768px - 1024px)
Sidebar collapses to hamburger-triggered overlay, search results stack vertically instead of side-by-side, document table switches to compact card view with key fields visible, two-column layouts collapse to single column, upload zone remains full-width but with smaller padding, action buttons move to a dropdown menu per row to save horizontal space, sidebar width reduced with icon-only mode.

### Mobile (below 768px)
Hamburger menu as primary navigation, full-width cards for all content types, search bar fixed at top of viewport, document upload uses native file picker with compact progress indicator at top of screen, answer cards stack vertically with source citations expandable below each answer via tap, toast notifications slide in from bottom of screen, touch-friendly tap targets (minimum 44px by height), all modals are full-screen on mobile, pagination uses previous/next arrows only (no page number buttons), sticky header with hamburger toggle and tenant name truncated with ellipsis.
...
- Token refresh: access token expires mid-session -> auto-refresh -> continue with new token seamlessly
...
- Responsive testing: Chrome DevTools device emulation for all breakpoints
- Touch interaction testing: swipe gestures, tap targets, pinch-to-zoom on document viewer
...
- Form field labels associated with inputs via htmlFor
